...
def get_rotation_matrix(p, y, r):
    ...
    return np.array([[cy*cz + sy*sx*sz, -cy*sz + sy*sx*cz, sy*cx], [cx*sz, cx*cz, -sx], [-sy*cz + cy*sx*sz, sy*sz + cy*sx*cz, cy*cx]])
...
class CrossGestureDetector:
    ...
    def detect(self, pose_landmarks, img_w, img_h):
        self.just_triggered = False
        if pose_landmarks is None: return self._handle_absence()
        lm = pose_landmarks.landmark
        if any(lm[i].visibility < 0.3 for i in [13, 14, 15, 16]): return self._handle_absence()
        
        el, wr, er, wl = [np.array([lm[i].x * img_w, lm[i].y * img_h]) for i in [13, 16, 14, 15]]
        if wl[1] > el[1] + 80 and wr[1] > er[1] + 80: return self._handle_absence()
        
        vl, vr = wl - el, wr - er
        cross = intersect(el - vl * 0.2, el + vl * 1.5, er - vr * 0.2, er + vr * 1.5)
        angle = math.degrees(math.acos(np.clip(np.dot(vl, vr) / (np.linalg.norm(vl) * np.linalg.norm(vr) + 1e-6), -1.0, 1.0)))

        if (cross or np.linalg.norm(wl - wr) < (np.linalg.norm(vl) + np.linalg.norm(vr)) * 0.4) and (30.0 < angle < 160.0):
            now = time.time()
...
class FaceTracker:
    ...
    def update(self, face_results, img_w, img_h, f, cx, cy):
        ...
        v_up /= (np.linalg.norm(v_up) + 1e-8)
        v_forward = np.cross(v_right, v_up)
        v_forward /= (np.linalg.norm(v_forward) + 1e-8)
        
        R_screen = np.column_stack((v_right, -v_up, -v_forward))
        cx_val = math.sqrt(R_screen[1, 0]**2 + R_screen[1, 1]**2)
        p, y, r = (math.atan2(-R_screen[1, 2], cx_val), math.atan2(R_screen[0, 2], R_screen[2, 2]), math.atan2(R_screen[1, 0], R_screen[1, 1])) if cx_val > 1e-6 else (math.atan2(-R_screen[1, 2], 0.0), math.atan2(-R_screen[2, 0], R_screen[0, 0]), 0.0)
...
def render_quad_3d(frame, canvas, T, rot, w_3d, h_3d, f, cx, cy, color, draw_style='full', alpha=0.75, draw_axes=False):
    ...
    img_corners = []
    for pt in local_pts:
        px_py = project_point(R.dot(pt) + T, f, cx, cy)
        if px_py is None: return frame
    ...
    try:
        ...
        mask_3ch = cv2.merge([warped_mask]*3) / 255.0
        frame = np.where(mask_3ch > 0.01, cv2.addWeighted(warped, alpha, frame, 1.0 - alpha, 0), frame)
    except:
        pass
        
    pts = img_corners.astype(int)
    if draw_style == 'full':
        for idx in range(4): cv2.line(frame, tuple(pts[idx]), tuple(pts[(idx + 1) % 4]), color, 1, cv2.LINE_AA)
    elif draw_style == 'corners':
        c_len = 25
        for idx in range(4):
            curr, nxt, prv = pts[idx], pts[(idx + 1) % 4], pts[(idx - 1) % 4]
            vn = (nxt - curr) / (np.linalg.norm(nxt - curr) + 1e-6)
            vp = (prv - curr) / (np.linalg.norm(prv - curr) + 1e-6)
            cv2.line(frame, tuple(curr), tuple((curr + vn * c_len).astype(int)), color, 2, cv2.LINE_AA)
            cv2.line(frame, tuple(curr), tuple((curr + vp * c_len).astype(int)), color, 2, cv2.LINE_AA)
            
    if draw_axes:
        op = project_point(T, f, cx, cy)
        if op:
            for vec, col in [([40, 0, 0], C_RED), ([0, -40, 0], C_GREEN), ([0, 0, 40], C_BLUE)]:
                pt_2d = project_point(T + R.dot(np.array(vec)), f, cx, cy)
...
class VirtualKeyboard:
    ...
    def get_key_rects(self):
        rects, margin_x, margin_y, key_h, gap = [], 8, 70, 42, 5
        total_w = self.canvas_w - margin_x * 2
        for r_idx, row in enumerate(self.rows):
            y = margin_y + r_idx * (key_h + gap)
            if r_idx == 3:
                sw, dw, ew = total_w // 2 - gap, total_w // 4 - gap, total_w // 4 - gap
                rects.extend([(margin_x, y, sw, key_h, "SPACE"), (margin_x + sw + gap, y, dw, key_h, "DEL"), (margin_x + sw + gap + dw + gap, y, ew, key_h, "ENTER")])
            else:
                n = len(row)
                kw = (total_w - gap * (n - 1)) // n
                ox = margin_x + (r_idx % 2) * 10
                for k_idx, lbl in enumerate(row): rects.append((ox + k_idx * (kw + gap), y, kw, key_h, lbl))
        return rects
    ...
    def draw_canvas(self):
        ...
        now = time.time()
        for x, y, w, h, lbl in self.get_key_rects():
    ...
    def handle_touch(self, local_finger, is_im_pinched):
        ...
        if -30.0 <= w <= 80.0:
            for x, y, width, height, lbl in self.get_key_rects():
    ...
    def get_local_finger_for_keyboard(self, index_tip_3d):
        return get_rotation_matrix(self.rot[0], self.rot[1], self.rot[2]).T.dot(index_tip_3d - self.T)
...
class SciFiScreen3D:
    def __init__(self, w_3d=300, h_3d=180):
        ...
        self.smooth_alpha_pos, self.smooth_alpha_rot = 0.25, 0.20
    ...
    def draw_canvas(self, current_mode=0):
        canvas = np.zeros((self.canvas_h, self.canvas_w, 3), dtype=np.uint8)
        for x in range(0, self.canvas_w, 20): cv2.line(canvas, (x, 0), (x, self.canvas_h), (25, 25, 10), 1)
        for y in range(0, self.canvas_h, 20): cv2.line(canvas, (0, y), (self.canvas_w, y), (25, 25, 10), 1)
...
def extract_hand_data_3d(landmarks, img_w, img_h, f, cx, cy):
    ...
    zs = Z_hand * (1.0 + np.array([lm.z for lm in landmarks]) * 1.2)
    xs = (np.array([lm.x for lm in landmarks]) * img_w - cx) * zs / f
    ys = (np.array([lm.y for lm in landmarks]) * img_h - cy) * zs / f
    ...
    pitch, yaw, roll = (math.atan2(R_hand[2, 1], R_hand[2, 2]), math.atan2(-R_hand[2, 0], sy), math.atan2(R_hand[1, 0], R_hand[0, 0])) if sy > 1e-6 else (math.atan2(-R_hand[1, 2], R_hand[1, 1]), math.atan2(-R_hand[2, 0], sy), 0.0)
...
def handle_interaction(screen, landmarks, pts_3d, hand_rot):
    ...
    if is_pinching:
        if not screen.is_dragging and np.linalg.norm(pinch_mid - screen.T) < 250.0:
    ...
    if not screen.is_dragging and not is_pinching:
        local = get_rotation_matrix(screen.rot[0], screen.rot[1], screen.rot[2]).T.dot(index_tip - screen.T)
        u, v, w = local
...
def draw_mode_switch_progress(frame, progress, current_mode, next_mode):
    ...
    overlay = frame.copy()
    cv2.rectangle(overlay, (cx - 160, cy - 100), (cx + 160, cy + 100), (0, 0, 0), -1)
...
def draw_cross_indicator(frame, pose_landmarks, img_w, img_h):
    ...
    lm = pose_landmarks.landmark
    if any(lm[i].visibility < 0.3 for i in [13, 14, 15, 16]): return frame
    
    pts = [(int(lm[i].x * img_w), int(lm[i].y * img_h)) for i in [13, 15, 14, 16]]
...
while cap.isOpened():
    ...
    if now - mode_switch_cooldown > MODE_SWITCH_COOLDOWN_TIME:
        cross_triggered, cross_progress = cross_detector.detect(pose_results.pose_landmarks, img_w, img_h)
    else:
    ...
    if results.multi_hand_landmarks:
        if cross_progress < 0.1:
            ...
            if finger_active and finger_local_pos is not None:
                ...
                laser_hit_w = T + get_rotation_matrix(rot[0], rot[1], rot[2]).dot(np.array([finger_local_pos[0], finger_local_pos[1], 0.0]))
                ...
                if idx_2d and laser_hit_2d:
                    ...
                    cv2.circle(frame, laser_hit_2d, 5, C_YELLOW, -1)
                    cv2.circle(frame, laser_hit_2d, 10, C_CYAN, 1, cv2.LINE_AA)
    else:
        my_screen_3d.is_dragging = False
    ...
    if cross_progress > 0.01:
        ...
        frame = draw_cross_indicator(frame, pose_results.pose_landmarks, img_w, img_h)

    if mode_transition_active and mode_transition_alpha > 0:
        flash = np.full_like(frame, [C_GREEN, C_ORANGE, C_MAGENTA][current_mode], dtype=np.uint8)
